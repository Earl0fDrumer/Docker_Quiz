...
#include <sstream> // For building custom messages
...
public:
  ...
  handleError(const oatpp::web::protocol::http::Status& status,
    ...
    if (status.code == 404) {
      ...
      funMessage = "Something went wrong, but don't worry! We're working on it. 🛠️";
    }
    ...
    auto bufferBody = oatpp::web::protocol::http::outgoing::BufferBody::createShared(bodyStream.str());

    // Return the response
    return oatpp::web::protocol::http::outgoing::Response::createShared(status, bufferBody);
  }
};
