...
void SE_MAT_Test::onRun() {
  ...
  runner.run(
      [this, &runner] {  // Keep this format, capture `runner`
        ...
        std::string s = "An ad hoc approach to maintaining software. "
        "It is a fire-fighting approach waiting for the "
        "problem to occur and then trying to fix it as "
        "quickly as possible.";
        ...
        s = "A simple working system implementing only a "
        "few basic features is built, thereafter many successive "
        "iterations/ versions are implemented and delivered to "
        "the customer.";
        OATPP_ASSERT(message->definitionC == s);
        s = "A methodology focused on iterative and incremental "
